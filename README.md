...
本节作业完成一个激光切割拼插盒。目标是将盒子的底板、长侧板和短侧板分别设计为可切割的 DXF 文件，通过榫卯式插接结构完成无螺丝装配，最终形成一个可以放置小物件的木质盒子。
...
各板件边缘设计了对应的拼插齿，使侧板可以与底板和相邻侧板互相咬合。底板上还加入了文字雕刻内容，用于标记小组成员信息。整体结构比较直接，便于激光切割后快速装配，也能观察切割缝、板厚和插接间隙对装配效果的影响。

### 3. 切割文件

- [底板 DXF](laser_cutting/assignment7/底板.dxf)
- [长侧板 1 DXF](laser_cutting/assignment7/长侧板1.dxf)
- [长侧板 2 DXF](laser_cutting/assignment7/长侧板2.dxf)
- [短侧板 1 DXF](laser_cutting/assignment7/短侧板1.dxf)
- [短侧板 2 DXF](laser_cutting/assignment7/短侧板2.dxf)

### 4. 拼插成果展示

#### 俯视效果

![激光切割拼插盒俯视图](images/assignment7/拼插演示图1.png)

#### 侧面效果

![激光切割拼插盒侧面图](images/assignment7/拼插演示图2.png)

### 5. 本阶段小结

这次激光切割作业让我理解了二维图纸和三维实物之间的关系。DXF 文件中看似只是几块平面板，但切割后通过插接结构可以组合成立体盒子。实际装配时，板材厚度、切割缝宽和插齿间隙都会影响松紧程度。后续如果继续优化，可以根据实际板材厚度微调卡槽尺寸，让拼插更顺畅，同时减少边缘焦痕对外观的影响。

---

## 八、第八节：期末项目——弱光压力触发智能灯带装置

### 1. 项目目标

期末项目完成的是一个弱光压力触发智能灯带装置。项目由光敏传感器、FSR 薄膜压力传感器、WS2812 灯带和 3D 打印外壳组成，目标是在环境较暗时，通过压力触发灯带点亮；再次触发可以关闭灯带，如果 30 秒内没有操作，系统会自动熄灭。

这个项目相比前面的单项练习更完整，需要同时完成外壳结构、传感器输入、灯带控制、程序判断和实物调试。中期项目保留下来的资料比较少，因此期末项目作为本学期最终展示重点。

### 2. 硬件与结构组成

- 控制板：运行 Arduino 程序，读取传感器并控制灯带。
- 光敏传感器：判断当前环境是强光、正常光还是弱光。
- FSR 薄膜压力传感器：作为触发输入，用来控制灯带开关。
- WS2812 灯带：作为主要输出，点亮时显示暖橙色。
- 3D 打印外壳：包括盒子、盖子和梯子形结构，用于固定和展示硬件。

### 3. 程序逻辑

程序使用 `Adafruit_NeoPixel` 库控制 WS2812 灯带，灯带数据引脚连接到 GPIO18。光敏传感器连接到 GPIO4，压力传感器连接到 GPIO5。

核心控制逻辑如下：

- 读取光敏传感器数值，判断当前是否为弱光或黑暗环境。
- 读取压力传感器数值，并区分无压力、轻微压力、中等压力和较大压力。
- 当检测到较大压力的上升沿时，如果当前灯带关闭，并且环境较暗，就点亮灯带。
- 如果灯带已经点亮，再次触发压力传感器则关闭灯带。
- 灯带点亮后，如果 30 秒没有新的操作，自动熄灭。

### 4. 源码与模型文件

- [期末代码](code/assignment8/期末代码.ino)
- [期末盒子 3D 打印图纸](fusion_models/assignment8/期末盒子3D打印图纸.stl)
- [期末盖子 3D 打印图纸](fusion_models/assignment8/期末盖子3D打印图纸.stl)
- [期末梯子 3D 打印图纸](fusion_models/assignment8/期末梯子3D打印图纸.stl)
- [期末演示视频](video/assignment8/期末演示.mp4)

### 5. 制作与调试过程

#### 调试过程

![期末项目调试过程图](images/assignment8/期末项目完成过程图失败图.jpg)

调试阶段先在面包板上连接传感器、灯带和控制板，通过串口输出观察光敏传感器和压力传感器读数。这个阶段主要遇到的问题是传感器阈值不稳定，需要根据实际环境重新判断“弱光”和“较大压力”的范围。

#### 结构装配

![期末项目完成过程图](images/assignment8/期末项目完成过程图.jpg)

外壳部分使用 3D 打印件组合，盒子和盖子用于收纳内部硬件，梯子形结构用于支撑或展示灯带。装配过程中需要考虑走线位置、传感器露出位置和灯带固定方式。

#### 最终实物

![期末项目实物图](images/assignment8/期末项目实物图.jpg)

最终作品可以在弱光环境下通过压力触发点亮灯带，灯带亮起后呈暖橙色。再次触发压力传感器可以关闭灯带，或者等待 30 秒自动关闭。

### 6. 期末项目小结

这个项目把本学期学到的多项能力结合在一起：既有 Arduino 程序编写和传感器读取，也有 WS2812 灯带控制，还有 3D 打印外壳和实物装配。调试过程中我认识到，硬件项目不能只看代码是否能编译，还要根据真实传感器数据不断调整阈值，并考虑结构设计是否方便接线和安装。

---

## 全学期课程总结

通过本学期《从代码到实物》的学习，我从最开始的 Gitee 仓库和网页记录，逐步完成了 3D 建模、3D 打印、ESP32 控制、墨水屏中期项目、EDA 与 PCB 设计、激光切割和期末综合项目。整个过程让我对“从数字文件到实体作品”的流程有了更直观的理解。

前面的作业更多是单项能力训练，例如 3D 手机支架训练建模和打印，WiFi 控制舵机训练 ESP32 网络控制，PCB 作业训练电路图和板图设计。期末项目则把传感器、灯带、程序逻辑和 3D 打印外壳放到同一个作品里，是对前面学习内容的一次综合应用。
